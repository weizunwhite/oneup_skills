...
- **Do NOT present cost/price as a research objective or innovation point.** Students' projects focus on solving problems through technology, not on cost optimization. When extracting content from a paper, skip cost-related objectives (e.g., "成本控制在X元以内") and cost-related innovation claims (e.g., "低成本方案"). Cost may appear briefly in background research slides as context (e.g., "现有产品价格较高，不适合学校使用"), but never as a highlighted metric or goal on testing/innovation slides.
- **No metaphors or analogies for technical components** (all grades). Never compare MCUs to "brains", cameras to "eyes", sensors to "noses", etc.
